...
void AssemblyWidget::on_loadButton_pressed()
{
    QSettings settings;
    ...
    {
        std::cerr << "No file available to be opened!" << std::endl;
        return;
    }
    ...
    {
        std::cerr << "Unable to import model!" << std::endl;
        return;
    }
    std::cout << "Loaded model from file " << selectedFilePath.toLocal8Bit().data() << std::endl;
    updateModelDimensions();
}

void AssemblyWidget::updateModelDimensions()
{
    QString dimensions;
    dimensions.sprintf("%.2f x %.2f x %.2f", modeler.getModelSize().X(), modeler.getModelSize().Y(), modeler.getModelSize().Z());
    dimensionLabel->setText(dimensions);
    std::cout << "Computed model dimensions " << dimensions.toUtf8().constData() << std::endl;
}

void AssemblyWidget::on_scaleButton_pressed()
{
    modeler.scale(scaleRatioSpinBox->value());
    updateModelDimensions();
}

void AssemblyWidget::on_processFileButton_pressed()
{
    bool ok;
    ...
    convertFile(tmpfile.fileName(), voxelizationResolution);
}
...
void AssemblyWidget::convertFile(const QString &filePath, int voxelizationResolution)
{
